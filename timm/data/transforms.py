...
from timm.data import paired_transforms_tv04 as transforms
...
class ToNumpy:
    ...
    def __call__(self, pil_img, pil_img1=None):
        np_img = np.array(pil_img, dtype=np.uint8)
        if np_img.ndim < 3:
            np_img = np.expand_dims(np_img, axis=-1)
        np_img = np.rollaxis(np_img, 2)  # HWC to CHW
        if pil_img1 is not None:
            np_img1 = np.array(pil_img1, dtype=np.uint8)
            if np_img1.ndim < 3:
                np_img1 = np.expand_dims(np_img1, axis=-1)
            np_img1 = np.rollaxis(np_img1, 2)  # HWC to CHW
            return np_img, np_img1
        return np_img
...
class ToTensor:
    ...
    def __call__(self, pil_img, pil_img1=None):
        ...
        if pil_img1 is not None:
            ...
            if np_img1.ndim < 3:
                np_img1 = np.expand_dims(np_img1, axis=-1)
            np_img1 = np.rollaxis(np_img1, 2)
            return torch.from_numpy(np_img).to(dtype=self.dtype), torch.from_numpy(np_img1).to(dtype=self.dtype)
