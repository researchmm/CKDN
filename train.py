...
parser.add_argument('--decay-epochs', type=int, default=5, metavar='N',
...
def main():
    ...
    if args.num_gpu > 1:
        ...
        model = torch.nn.parallel.DistributedDataParallel(model.cuda(),device_ids=[args.local_rank], output_device=args.local_rank, broadcast_buffers=True,find_unused_parameters=True)
    ...
    dataset_train = Dataset(args.data,train_data)
    ...
    dataset_eval = Dataset(args.data,val_data)
...
def loss_fn(o1,o2,o3,o4,t1,t2,epoch):
    t3 = 1.0/(1+torch.pow(10,(t1-t2)*1000/400))
    d1 = ((o1-t1)**2).mean()
    d2 = ((o2-t1)**2).mean()
    d3 = ((o3-t3)**2).mean()*0.5
    d4 = ((o4)**2).mean()*30.0
...
def validate(model, loader, args, log_suffix=''):
    ...
    with torch.no_grad():
        ...
        outputs = np.concatenate(outputs, axis=0).squeeze()#.reshape((-1,4)).transpose()
        targets = np.concatenate(targets, axis=0).squeeze()#.reshape((-1,4)).transpose()
        outputs = outputs.reshape((25,-1))
        targets = targets.reshape((25,-1))
        SRCCs = 0
        PLCCs = 0
        for i in range(25):
            SRCC,PLCC = compute_metrics(outputs[i], targets[i], is_np=True)
            SRCCs+=SRCC
            PLCCs+=PLCC
        SRCCs/=0.25
        PLCCs/=0.25
        if args.local_rank==0:
            print('SRCCs={:.2f},PLCCs={:.2f}'.format(SRCCs,PLCCs))
        scores = SRCCs+PLCCs


    metrics = OrderedDict([('loss', scores), ('prec1', scores), ('prec5', scores)])
